...
type SubscribeListService struct {
...
// 判断是否有上一页或者下一页
func (service *SubscribeListService) HaveNextOrLast() (next bool, last bool) {
	if service.Page <= 1 {
		last = false
	} else {
		last = true
...
func (service *SubscribeListService) Pages() (int, *serializer.Response) {
	var count int
	if service.Count == 0 || service.All == 0 {
		return 0, nil
	}
...
func (service *SubscribeListService) Pull(create uint) *serializer.Response {
	...
	if err := DB.Find(&UserSubscribe).Error; err != nil {
		return &serializer.Response{
			Status: 500,
			Msg:    "获取失败",
		}
	}
	service.Count = len(UserSubscribe)
...
func (service *SubscribeListService) Counts() int {
	return service.Count
}
func (service *SubscribeListService) Response() interface{} {
